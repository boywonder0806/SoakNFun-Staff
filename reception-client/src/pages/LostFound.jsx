...
export default function LostFound({ onUnclaimedCount }) {
  ...
  const visible = items.filter(i => {
    ...
    if (search) {
      ...
      return (i.itemDescription || '').toLowerCase().includes(q)
...
function LogItemModal({ onClose, onCreated }) {
  const [form, setForm] = useState({
    itemDescription: '', locationFound: '', foundDate: today(), ownerName: '', ownerContact: '', notes: '',
  });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.itemDescription.trim()) { setError('Item description is required.'); return; }
    ...
    <Modal title="Log Found Item" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input className="field" placeholder="e.g. Blue water bottle, prescription glasses…" value={form.itemDescription} onChange={set('itemDescription')} required />
        </div>
...
function ItemDetailModal({ item, onClose, onUpdated }) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const { data } = await api.patch(`/reception/lost-found/${item.id}`, { status, ownerName, ownerContact, notes });
      onUpdated(data.item);
    } catch {}
    setSaving(false);
  }
