...
4. search_line_items — keyword search across EVERY line item in every order, finding all name variants automatically. ALWAYS use this when the user asks about a specific item by name (cheeseburger, fries, admission ticket, funnel cake, etc.). Item names in RocketRez have suffixes like "(BB Employee)", combo names, location prefixes — this tool handles all of that automatically and sums across all variants. Never try to answer item-count questions from the top-30 list in get_order_summary. IMPORTANT: If a search returns 0 results, do NOT immediately tell the user nothing was found — automatically retry with a shorter, broader keyword (e.g. "phone case" → retry "phone"; "funnel cake" → retry "funnel"; "admission ticket" → retry "admission") before concluding there were no sales. Only report zero after at least one retry with a broader term.
