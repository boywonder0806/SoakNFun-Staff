    ...
    <div className="relative space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap print:hidden">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-semibold text-white bg-az hover:bg-az-dark rounded-lg shrink-0"
        >
          🖨️ Print
        </button>
      </div>
      ...
      <div className="hidden print:block">
        ...
        <p className="text-xs text-gray-500">
          {params.startDate === params.endDate ? shortDate(params.startDate) : `${shortDate(params.startDate)} – ${shortDate(params.endDate)}`}
          {' · '}{params.park === 'BB' ? 'Blue Bayou' : params.park === 'GI' ? 'Gulf Islands' : 'Both Parks'}
        </p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 print:hidden">
        <KpiTile
          label="Total Cash Collected" icon="💵" accent
          value={money(kpis.totalCash)}
          sub="net of same-day cash refunds"
        />
        <KpiTile
          label="All Tenders Collected" icon="🧾"
          value={money(kpis.totalAll)}
          sub="every payment method"
        />
        <KpiTile
          label="Cashiers / Engines" icon="🧑‍💼"
          value={number(kpis.cashierCount)}
          sub="distinct salespeople"
        />
        <KpiTile
          label="Orders Processed" icon="📦"
          value={number(kpis.orderCount)}
          sub="with a payment in range"
        />
      </div>
